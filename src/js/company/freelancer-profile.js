...
function buildServiceCard(service) {
  ...
  return `
    <div class="group bg-slate-900 rounded-xl overflow-hidden border border-slate-800 hover:border-purple-600 transition-all duration-300 hover:scale-[1.04] hover:shadow-xl opacity-0 translate-y-4">
      <img src="${escapeHtml(image)}" class="h-48 w-full object-cover group-hover:scale-110 transition-transform duration-500 ease-out"/>
...
function applyServicesReveal() {
  const cards = servicesEl?.querySelectorAll(".group") || [];
...
async function fetchFreelancer() {
  ...
  try {
    ...
    const photo = buildStorageUrl(user?.photo || data?.freelancer_profile?.photo) || "/logo.jpeg";
