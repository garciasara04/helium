...
const listEl = document.getElementById("companyReviewsList");
const emptyEl = document.getElementById("companyReviewsEmpty");
const loadingEl = document.getElementById("companyReviewsLoading");
...
function buildCard(order, highlight) {
  ...
  const imageUrl = freelancerUser?.photo || "https://via.placeholder.com/300";
  ...
  return `
    <form class="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4 ${highlight ? "ring-2 ring-violet-500" : ""} ${disabledClass}" data-order-id="${escapeHtml(order.id)}">
      <div class="flex items-center gap-4">
        <img src="${escapeHtml(imageUrl)}" class="w-12 h-12 rounded-full object-cover"/>
        <div>
